...
def visualize_demonstration(shapenet_path: str, gripper_mesh_path: str):
    ...
    rr.log("world", rr.ViewCoordinates.RIGHT_HAND_Z_DOWN, static=True)  # Set an up-axis
    camera_data = demo_data['observations'][0]['camera_0']
    print(camera_data['camera_intrinsics']['matrix'])
    rr.log(
        "world/camera0",
        rr.Pinhole(
    ...
    for i, obs in enumerate(demo_data['observations']):
        for camera_id, camera_data in obs.items():
            rr.log(f"world/camera0/color", rr.Image(camera_data['color']))
            rr.log(f"world/camera0/depth", rr.DepthImage(camera_data['depth']))
            rr.log(f"world/camera0", rr.Transform3D(
            ...
            rr.log(f"world/point_cloud", rr.Points3D(positions=points, colors=colors))
