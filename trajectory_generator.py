...
class TrajectoryGenerator:
    def __init__(self, resolution: float = 0.01, angle_resolution: float = np.pi/60):
        """
        Initialize trajectory generator.
        
        Args:
            resolution: Spatial resolution in meters (default: 1cm)
            angle_resolution: Angular resolution in radians (default: 3 degrees)
        """
    ...
    def interpolate_trajectory(self,
                             ...
                             metadata: List[Any],
                             method: Literal['linear', 'cubic', 'spherical'] = 'linear'
                             ) -> Tuple[List[np.ndarray], List[Any]]:
        """
        Interpolate trajectory between waypoints using specified method.
        
        Args:
            waypoints: List of 4x4 transformation matrices
            metadata: List of metadata associated with each waypoint
            method: Interpolation method to use
            
        Returns:
            Tuple of (interpolated poses, interpolated metadata)
        """
        if method == 'linear':
            return self._linear_interpolation(waypoints, metadata)
        elif method == 'cubic':
            return self._cubic_interpolation(waypoints, metadata)
        elif method == 'spherical':
            return self._spherical_interpolation(waypoints, metadata)
        else:
    ...
    def _linear_interpolation(self,
        ...
        for i in range(len(waypoints) - 1):
            ...
            steps = max(
                int(pos_diff / self.resolution),
                int(rot_diff / self.angle_resolution)
            )
            steps = max(steps, 1)
            ...
            for t in np.linspace(0, 1, steps):
                ...
                interp_quat = self._slerp(start_quat, end_quat, t)
    ...
    def _cubic_interpolation(self,
        ...
        num_steps = int(total_length / self.resolution)
        ...
        for t in t_interp:
            ...
            rot = t3d.quaternions.quat2mat(
                self._slerp(start_quat, end_quat, frac)
            )
    ...
    def _spherical_interpolation(self,
                               waypoints: List[np.ndarray],
                               metadata: List[Any]
                               ) -> Tuple[List[np.ndarray], List[Any]]:
        ...
        for i in range(len(waypoints) - 1):
            ...
            steps = max(int(angle_diff / self.angle_resolution), 1)
            ...
            for t in np.linspace(0, 1, steps):
                ...
                end_quat = t3d.quaternions.mat2quat(end_pose[:3, :3])
                rot = t3d.quaternions.quat2mat(
                    self._slerp(start_quat, end_quat, t)
                )
