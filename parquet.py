...
for recording_file in tqdm(list(range(num_recordings)), desc="Processing recording files"):
    ...
    episode_df.write_parquet(f"dataset", row_group_size=1, compression="zstd", partition_by="meta_episode_number")
    # episode_df.write_parquet(f"dataset", row_group_size=1, compression="uncompressed", partition_by="meta_episode_number")
